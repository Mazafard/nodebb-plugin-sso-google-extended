...
const constants = Object.freeze({
	name: 'Google',
	admin: {
		route: '/plugins/sso-google-extended',
...
Google.init = async function (data) {
	...
	hostHelpers.setupPageRoute(data.router, '/deauth/google', [data.middleware.requireUser], (req, res) => {
		res.render('plugins/sso-google-extended/deauth', {
			service: 'Google',
		});
	});
	...
	const loadedSettings = await meta.settings.get('sso-google-extended');
	...
	Google.settings.buttonPosition = loadedSettings.buttonPosition || 'top';
...
Google.filterConfigGet = function (data) {
	data['sso-google-extended'] = {
		enabled: Boolean(Google.settings.id && Google.settings.secret),
		buttonPosition: Google.settings.buttonPosition || 'top',
...
Google.filterAuthInit = function (strategies) {
	if (Google.settings.id && Google.settings.secret) {
		...
		strategies.push({
			...
			labels: {
				login: '[[sso-google-extended:button.sign_in_with_google]]',
				register: '[[sso-google-extended:button.sign_up_with_google]]',
			},
			color: '#1DA1F2',
...
Google.addAdminMenuItem = function (custom_header) {
	custom_header.authentication.push({
		...
		icon: constants.admin.icon,
		name: '[[sso-google-extended:admin.menu.title]]',
	});
